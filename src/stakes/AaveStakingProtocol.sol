...
contract AaveStakingProtocol is IStakingProtocol {
    ...
    function deposit(address asset, uint256 amount) external payable override {
        IERC20Minimal(asset).approve(aave, amount);
        IAaveV3Minimal(aave).supply(asset, amount, address(this), referralCode);
    }

    function withdraw(address asset, address to, uint256 amount) external override {
        IERC20Minimal aToken = IERC20Minimal(IAaveV3Minimal(aave).getReserveData(asset).aTokenAddress);
